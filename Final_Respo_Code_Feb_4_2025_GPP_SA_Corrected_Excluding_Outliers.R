...
respo_data <- respo_data[!is.na(respo_data$GPP_SA_Corrected_cm2_hr) & 
...
T0_M_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
leveneTest(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment, 
           data = filtered_data_MCAP_T0)
...
T0_M_plot <- ggplot(data = filtered_data_MCAP_T0, 
  ...
  geom_text(data = counts, aes(x = Nutrient_Treatment, y = 0.5, label = paste0("n=", n)), 
...
# Fit the model without the outliers
T0_P_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
T1_M_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
T1_M_plot <- ggplot(data= filtered_data_MCAP_T1, 
  ...
  geom_boxplot(position = position_dodge(width = 0.8), alpha = 0.7) +
  geom_jitter(aes(color = Nutrient_Treatment), position = position_dodge(width = 0.8), alpha = 1) + 
  geom_text(data = counts, aes(x = Nutrient_Treatment, y = 0.25, label = paste0("n=", n)), 
            inherit.aes = FALSE, vjust = 0) +
...
# Fit the model without the outliers
T1_P_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
# Fit the model without the outliers
T2_M_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
...
# Testing normality and homogeneity
shapiro.test(residuals)
leveneTest(GPP_SA_Corrected_cm2_hr ~ interaction(Nutrient_Treatment, Temp_Treatment), 
           data = filtered_data_MCAP_T2)
...
T2_M_plot <- ggplot(filtered_data_MCAP_T2, 
  ...
  geom_text(data = counts, aes(x = Temp_Treatment, y = 0.25, label = paste0("n=", n)), 
            inherit.aes = FALSE, vjust = 0) +
...
#Fit the model without outliers
T2_P_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
                  data = respo_data %>%
                    filter(Species == "PCOMP", Time_Point == "T2"))
summary(T2_P_lmer)

# Step 3: Perform ANOVA and Tukey posthoc test
...
T2_P_plot <- ggplot(filtered_data_PCOMP_T2, 
  ...
  geom_boxplot(position = position_dodge(width = 0.8), alpha = 0.7) +
  geom_jitter(aes(color = Nutrient_Treatment), position = position_dodge(width = 0.8), alpha = 1) + 
  geom_text(data = counts, aes(x = Temp_Treatment, y = -0.2, label = paste0("n=", n)), 
            inherit.aes = FALSE, vjust = 0) +
  facet_wrap(~ Nutrient_Treatment, ncol=4) +
...
# Fit the model without the outliers
T3_M_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
...
# Fit the model without the outliers
T3_P_lmer <- lmer(GPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
                  data = filtered_data_PCOMP_T3)
summary(T3_P_lmer)
T3_P_lmer_gen <- lmer(GPP_SA_Corrected_cm2_hr ~ Temp_Treatment + (1|Colony_ID), 
                      data = filtered_data_PCOMP_T3)

# Perform ANOVA. Since interaction is not significant, no post-hoc test is done
...
T0_M_plot_modified <- T0_M_plot + 
  ...
  labs(subtitle = "T0", y = expression("GPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) +  
...
joint_title <- ggdraw() + 
  draw_label(expression(italic("Montipora capitata") ~ "Gross Primary Production", fontface = "bold", size = 14))
...
joint_title_P <- ggdraw() + 
  draw_label(expression(italic("Porites compressa") ~ "Gross Primary Production", fontface = "bold", size = 14))
...
T2_M_plot_modified <- T2_M_plot + 
  ...
  labs(subtitle = "T2", y = expression("GPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) + 
  theme(legend.position = "none", plot.title = element_blank()) 
...
T0_M_plot_modified <- T0_M_plot + 
  scale_y_continuous(limits = c(-0.25, 2.0), breaks = seq(-0.25, 2.0, by = 0.25)) + 
  labs(subtitle = expression(bold("A) T0")), y = expression("GPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) +
  theme(legend.position = "none", plot.title = element_blank())
...
T2_M_plot_modified <- T2_M_plot + 
  scale_y_continuous(limits = c(-0.25, 2.0), breaks = seq(-0.25, 2.0, by = 0.25)) + 
  labs(subtitle = expression(bold("C) T2")), y = expression("GPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) +
  theme(legend.position = "none", plot.title = element_blank())
...
T0_P_plot_modified <- T0_P_plot + 
  scale_y_continuous(limits = c(-0.25, 5.0), breaks = seq(-0.25, 5.0, by = 0.25)) + 
  labs(subtitle = expression(bold("A) T0")), y = expression("GPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) +
  theme(legend.position = "none", plot.title = element_blank())
