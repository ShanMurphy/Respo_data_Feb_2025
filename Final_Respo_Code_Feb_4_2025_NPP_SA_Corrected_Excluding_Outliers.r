...
T0_M_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
leveneTest(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment, 
                               data = filtered_data_MCAP_T0)
...
# Fit the model without the outliers
T0_P_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
T1_M_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
T1_M_plot <- T1_M_plot +
  geom_signif(
    comparisons = list(c("Control", "Guano"), 
                       c("Guano", "Inorganic")),
    annotations = c("**", "*"),  
    y_position = c(1.5, 1.6),  # Adjust as needed for better placement
    tip_length = 0.02,
    size = 0.5,
    textsize = 7,
    color = "black"
...
# Fit the model without the outliers
T1_P_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment + (1|Colony_ID), 
...
# Fit the model without the outliers
T2_M_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
...
# Testing normality and homogeneity
shapiro.test(residuals)
leveneTest(NPP_SA_Corrected_cm2_hr ~ interaction(Nutrient_Treatment, Temp_Treatment), 
                               data = filtered_data_MCAP_T2)
...
#Fit the model without outliers
T2_P_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
...
# Fit the model without the outliers
T3_M_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
...
# Fit the model without the outliers
T3_P_lmer <- lmer(NPP_SA_Corrected_cm2_hr ~ Nutrient_Treatment * Temp_Treatment + (1|Colony_ID), 
                  data = filtered_data_PCOMP_T3)
summary(T3_P_lmer)
T3_P_lmer_gen <- lmer(NPP_SA_Corrected_cm2_hr ~ Temp_Treatment + (1|Colony_ID), 
                  data = filtered_data_PCOMP_T3)
...
T0_M_plot_modified <- T0_M_plot + 
  ...
  labs(subtitle = "T0", y = expression("NPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) +  
...
T2_M_plot_modified <- T2_M_plot + 
  ...
  labs(subtitle = "T2", y = expression("NPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) + 
  theme(legend.position = "none", plot.title = element_blank()) 
...
T0_M_plot_modified <- T0_M_plot + 
  scale_y_continuous(limits = c(-0.25, 2.0), breaks = seq(-0.25, 2.0, by = 0.25)) + 
  labs(subtitle = expression(bold("A) T0")), y = expression("NPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) +
  theme(legend.position = "none", plot.title = element_blank())
...
T2_M_plot_modified <- T2_M_plot + 
  scale_y_continuous(limits = c(-0.25, 2.0), breaks = seq(-0.25, 2.0, by = 0.25)) + 
  labs(subtitle = expression(bold("C) T2")), y = expression("NPP [µmol O"[2] ~ " cm"^"-2" ~ " hr"^"-1" ~ "]")) +
  theme(legend.position = "none", plot.title = element_blank())
...
ggplot(data = filtered_data_MCAP_T0, aes(x = Nutrient_Treatment, y = Slope_Light)) +
  geom_boxplot()
